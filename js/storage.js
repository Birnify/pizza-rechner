...
(function (global) {
  ...
  function writeRaw(data) {
    PZ.store.set(KEY, JSON.stringify(data));
  }
  ...
  function applyState(o) {
    ...
    // Fall je gebraucht hätten — jedes künftig gespeicherte Rezept enthält beide Felder
    // automatisch (s. js/state.js PZ.state Defaults). Normale Robustheit gegen kaputte/
    // unvollständige EXTERNE Importe (Teilen-Link, Rezepte-Backup) bleibt unangetastet
    // und unabhängig davon bestehen: set() in js/ui.js parsed per parseFloat() und
    // no-opt bereits bei NaN/null/undefined, ganz unabhängig von dieser Stelle hier.
    set.oil(state.oil); set.sugar(state.sugar);
    ...
    // Legacy-Fallback für Rezepte ohne flourTemp-Feld (vor v3.20.0) entfernt (v3.60.0,
    // Nutzerauftrag, analog zum oil/sugar-Fallback oben, v3.53.0): keine real existierenden
    // alten Rezepte mehr, die diesen Fall je gebraucht hätten — jedes künftig gespeicherte
    // Rezept enthält das Feld automatisch (s. js/state.js PZ.state Defaults).
    set.flourTemp(state.flourTemp);
    // Segmente
